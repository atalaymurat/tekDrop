...
const calcWorkDimData = (data) => {
  ...
  works.map((w, i) => {
    ...
    if (w.unit === "m2") {
      ...
      w.dimensions.map((d, n) => {
        let dm2 = (d.length * d.width) / 1000000;
        totalM2 = dm2 * d.quanty;
      ...
      let totalPrice = sumDimF3 * w.price.val;
    ...
    if (w.unit === "m") {
      ...
      w.dimensions.map((d, n) => {
        let dmL = d.length / 1000;
        let tLength = formNumber(dmL * d.quanty);
        sumM += tLength;
        sumQuanty += d.quanty;
      ...
      works[i].totalQuanty = sumQuanty;
      let totalPrice = sumM * w.price.val;
      works[i].workTotalPrice = {
        val: formPrice(totalPrice),
        cur: w.price.cur,
      };
    ...
    if (w.unit === "adet" || w.unit === "takım") {
      ...
      let totalPrice = sum * w.price.val;
      works[i].workTotalPrice = {
        val: formPrice(totalPrice),
        cur: w.price.cur,
      };
  ...
  console.log("DATA FROM CALC **********", JSON.stringify(data, null, 4));
...
const calcGrandTotal = (data) => {
  ...
  gTotalTL = offerNetTotalPrice.TL + offerKdvPrice.TL;
...
const calcDiscount = (data) => {
  ...
  let disEUR = 0;
  if (discount > 0) {
    disTL = (data.offerTotalPrice.TL * discount) / 100;
    disUSD = (data.offerTotalPrice.USD * discount) / 100;
    disEUR = (data.offerTotalPrice.EUR * discount) / 100;
    ...
    console.log("DISCOUNT PRICE ********", data.offerDiscountPrice)
    return;
  }
